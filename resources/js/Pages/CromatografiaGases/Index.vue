...
const openModalEditUploadedForm = (cromatografiaGas) => {
  ...
  editUploadedDocForm.pozo_id = cromatografiaGas.pozo_id
...
<template>
  <div>
    ...
    <div class="flex items-center mb-6">
      ...
      <button v-if="can.deleteCromatografiaGas && cromatografiaGases.data.length !== 0 && !isTrashed" class="btn-secondary" type="button" :disabled="!selectAllCromGas && !selected.length" @click="removeSelectedItems">
        <span>Borrar</span>
        <span class="hidden md:inline">&nbsp;Elementos Seleccionados</span>
      </button>
      <button v-if="can.restoreCromatografiaGas && cromatografiaGases.data.length !== 0 && isTrashed" class="btn-secondary" type="button" :disabled="!selectAllCromGas && !selected.length" @click="restoreSelectedItems">
            ...
            <td class="px-6 py-4 border-solid border border-gray-200">
              ...
              <div v-else>
                <div class="flex items-center leading-snug">
                  <a class="text-yellow-400 hover:underline focus:text-yellow-500 cursor-pointer" @click="[docName = cromatografiaGas.documento.usrName ,viewPdfModal = true]">
